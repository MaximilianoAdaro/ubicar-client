...
class RutasLayer extends React.PureComponent<TVectorLayerComponentProps> {
    ...
    componentDidMount() {
        ...
        this.source = new VectorSource({
            url: "./geojson/Rutas Provinciales.geojson",
            format: new GeoJSON(),
    ...
    render() {
        return (
            <div className="custom-control custom-checkbox">
                <input type="checkbox" className="custom-control-input" id="customCheck4" checked={this.state.visible}
                       onChange={() => this.setState({visible:!this.state.visible})}/>
                <label className="custom-control-label" htmlFor="customCheck4">Rutas</label>
            </div>
        )
